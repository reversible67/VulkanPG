...
void VulkanExampleBase::getBuffer(void *data, vks::Buffer &buffer, VkDeviceSize size)
{
	if (buffer.memoryPropertyFlags == VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT)
	{
		...
		vulkanDevice->copyBuffer(&buffer, &stagingBuffer, queue);

		stagingBuffer.map();
		memcpy(data, stagingBuffer.mapped, size);
		stagingBuffer.unmap();
...
void VulkanExampleBase::updateBuffer(vks::Buffer &buffer, VkDeviceSize size, void *data)
{
	if (buffer.memoryPropertyFlags == VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT)
	{
		vks::Buffer stagingBuffer;
		...
		vulkanDevice->copyBuffer(&stagingBuffer, &buffer, queue);
...
void VulkanExampleBase::createBuffer(vks::Buffer &buffer, VkDeviceSize size, VkBufferUsageFlags usageFlags, VkMemoryPropertyFlags memoryPropertyFlags, void *data)
{
	if (memoryPropertyFlags == VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT && data)
	{
		vks::Buffer stagingBuffer;
...
void VulkanExampleBase::nextFrame()
{
	...
	sumTime += tDiff;
	++timeFrameCounter;
	if (timeFrameCounter == 100)
	{
		frameTime = sumTime / 100.0;
		sumTime = 0.0;
		timeFrameCounter = 0;
	}
...
void VulkanExampleBase::updateOverlay()
{
	...
	ImGui::Text("%.2f ms/frame (%.1d fps)", frameTime, lastFPS);
	ImGui::Text("Timings %.2f %.2f %.2f %.2f %.2f", timings[0], timings[1], timings[2], timings[3], timings[4]);
	ImGui::Text("Compute %.2f %.2f", timings[5], timings[7]);
...
VulkanExampleBase::VulkanExampleBase(bool enableValidation)
{
	...
	commandLineParser.add("scene", { "--scene" }, 1, "Scene");
	...
	if (commandLineParser.isSet("scene")) {
		settings.scene = commandLineParser.getValueAsInt("scene", 0);
	}
...
void VulkanExampleBase::buildComputeCommandBuffers() {}
...
void VulkanExampleBase::handleMouseMove(int32_t x, int32_t y)
{
	...
	//	camera.translate(glm::vec3(-0.0f, 0.0f, dy * .005f));
		envRot = glm::rotate(envRot, 0.01f * dx, glm::vec3(0.0f, 1.0f, 0.0f));
		envRot = glm::rotate(envRot, -0.01f * dy, glm::vec3(1.0f, 0.0f, 0.0f));
		viewUpdated = true;
		envUpdated = true;
	}
